...
#define BACKUP2_DIVIDER_Y           22
#define BACKUP2_TITLE_Y             22
#define BACKUP2_BAT_X               (GUI_SCREEN_WIDTH - 22)

#define BACKUP2_CARD_ROW_Y          52
#define BACKUP2_CARD_ROW_X          16
#define BACKUP2_CARD_NAME_X         (GUI_SCREEN_WIDTH - 16)

#define BACKUP2_CUR_LABEL_Y         88
#define BACKUP2_PERCENT_Y           120
#define BACKUP2_SIZE_Y              148
#define BACKUP2_BAR_Y               172

#define BACKUP2_PATH_BOX_Y          256
#define BACKUP2_PATH_LINE0_Y        235
#define BACKUP2_PATH_LINE1_Y        264

#define BACKUP2_WARN_Y              (GUI_SCREEN_HEIGHT - 18)

#define BACKUP2_COLOR_LABEL         make_color(0x99, 0x99, 0x99)
#define BACKUP2_COLOR_VOL           make_color(0x4A, 0x9E, 0xFF)
...
typedef struct {
    ...
    u8 card_total;
    char card_name[8];
    char size_done[16];     // 如 "5.9 GB"
    char size_total[16];    // 如 "8.6 GB"
    char path_dir[32];
    char path_vol[24];
...
static u8 backup2_bat_level_from_percent(u8 percent)
{
    if (percent <= 20) {
        return 1;
    }
...
static void backup2_update_battery(void)
{
    f_backup2_t *f = (f_backup2_t *)func_cb.f_cb;
    u8 level;
    ...
    level = backup2_bat_level_from_percent(sys_cb.vbat_percent);
...
static void backup2_update_display(void)
{
    f_backup2_t *f = (f_backup2_t *)func_cb.f_cb;
    char buf[48];
    s16 card_w;
    ...
    if (f->txt_card_name) {
        ...
        compo_textbox_set_location(f->txt_card_name,
                                   BACKUP2_CARD_NAME_X - card_w, BACKUP2_CARD_ROW_Y, 0, 0);
    }
    ...
    if (f->txt_size_done && f->txt_size_total) {
        ...
        compo_textbox_set_location(f->txt_size_done, start_x, BACKUP2_SIZE_Y, 0, 0);
        compo_textbox_set_location(f->txt_size_total, start_x + done_w, BACKUP2_SIZE_Y, 0, 0);
    }
...
compo_form_t *func_backing_up_2_page_form_create(void)
{
    ...
    compo_picturebox_set_pos(f->pic_divider, GUI_SCREEN_CENTER_X, BACKUP2_DIVIDER_Y);

    f->txt_title = compo_textbox_create(frm, 24);
    compo_textbox_set_location(f->txt_title, GUI_SCREEN_CENTER_X, BACKUP2_TITLE_Y, 0, 0);
    ...
    f->bat_level = backup2_bat_level_from_percent(sys_cb.vbat_percent);
    f->pic_bat = compo_picturebox_create(frm, s_bat_level_res[f->bat_level - 1]);
    compo_picturebox_set_pos(f->pic_bat, BACKUP2_BAT_X, BACKUP2_TITLE_Y);

    /* 第 N / M 张 + 卡名 */
    f->txt_card_row = compo_textbox_create(frm, 24);
    compo_textbox_set_location(f->txt_card_row, BACKUP2_CARD_ROW_X, BACKUP2_CARD_ROW_Y, 0, 0);
    ...
    compo_textbox_set_location(f->txt_card_name, BACKUP2_CARD_NAME_X, BACKUP2_CARD_ROW_Y, 0, 0);
    ...
    compo_textbox_set_location(txt, GUI_SCREEN_CENTER_X, BACKUP2_CUR_LABEL_Y, 0, 0);
    compo_textbox_set_autosize(txt, true);
    compo_textbox_set_align_center(txt, true);
    compo_textbox_set_font(txt, UI_BUF_FONT_BIN_FONT_SIZE_11_BIN);
    compo_textbox_set_forecolor(txt, BACKUP2_COLOR_LABEL);
    compo_textbox_set(txt, i18n[STR_CURRENT_CARD]);

    /* 百分比 */
    f->txt_percent = compo_textbox_create(frm, 8);
    compo_textbox_set_location(f->txt_percent, GUI_SCREEN_CENTER_X, BACKUP2_PERCENT_Y, 0, 0);
    ...
    compo_textbox_set_forecolor(f->txt_size_total, BACKUP2_COLOR_LABEL);

    /* 进度条：0.bin ~ 100.bin */
    f->pic_bar = compo_picturebox_create(frm, UI_BUF_IMAGE_BIN_0_BIN);
    compo_picturebox_set_pos(f->pic_bar, GUI_SCREEN_CENTER_X, BACKUP2_BAR_Y);

    /* 底部路径卡片 */
    f->pic_path = compo_picturebox_create(frm, UI_BUF_IMAGE_BIN_GROUP_3_BIN);
    compo_picturebox_set_pos(f->pic_path, GUI_SCREEN_CENTER_X, BACKUP2_PATH_BOX_Y);

    {
        ...
        compo_textbox_set_location(txt_save, 20, BACKUP2_PATH_LINE0_Y, 0, 0);
        ...
        compo_textbox_set_location(txt, path_x, BACKUP2_PATH_LINE0_Y, 0, 0);
        compo_textbox_set_autosize(txt, true);
...

    {
        ...
        txt = compo_textbox_create(frm, 32);
        compo_textbox_set_location(txt, 20, BACKUP2_PATH_LINE1_Y, 0, 0);
        compo_textbox_set_autosize(txt, true);
        compo_textbox_set_align_center(txt, false);
        compo_textbox_set_font(txt, UI_BUF_FONT_BIN_FONT_SIZE_11_BIN);
        compo_textbox_set_forecolor(txt, BACKUP2_COLOR_VOL);
        compo_textbox_set(txt, buf);
    }

    /* 底部提示 */
    txt = compo_textbox_create(frm, 32);
    compo_textbox_set_location(txt, GUI_SCREEN_CENTER_X, BACKUP2_WARN_Y, 0, 0);
    compo_textbox_set_autosize(txt, true);
    compo_textbox_set_align_center(txt, true);
    compo_textbox_set_font(txt, UI_BUF_FONT_BIN_FONT_SIZE_11_BIN);
    compo_textbox_set(txt, i18n[STR_DO_NOT_REMOVE_MEDIA]);

    backup2_update_display();

    return frm;
}

static void func_backing_up_2_page_process(void)
{
    backup2_update_battery();
    func_process();
}

static void func_backing_up_2_page_message(size_msg_t msg)
{
    switch (msg)
    {
    case KU_BACK:
        func_cb.sta = FUNC_LOADING_2_PAGE;
        break;
...
void func_backing_up_2_page_enter(void)
{
    ...
    func_cb.f_cb = func_zalloc(sizeof(f_backup2_t));
    f = (f_backup2_t *)func_cb.f_cb;
    ...
    strcpy(f->size_done, "5.9 GB");
    strcpy(f->size_total, "8.6 GB");
    ...
    func_cb.frm_main = func_backing_up_2_page_form_create();
}

void func_backing_up_2_page_exit(void)
{
    func_cb.last = FUNC_LOADING_2_PAGE;
}

void func_backing_up_2_page(void)
{
    printf("%s\n", __func__);
    func_backing_up_2_page_enter();
    while (func_cb.sta == FUNC_BACKING_UP_2_PAGE)
    {
        func_backing_up_2_page_process();
        func_backing_up_2_page_message(msg_dequeue());
    }
    func_backing_up_2_page_exit();
}
