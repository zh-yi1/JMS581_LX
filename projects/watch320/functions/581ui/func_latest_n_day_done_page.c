...
#define LDONE_DIVIDER_Y             22
#define LDONE_TITLE_Y               22
#define LDONE_BAT_X                 (GUI_SCREEN_WIDTH - 22)

#define LDONE_ICON_Y                100

#define LDONE_STA_Y                 150
#define LDONE_SIZE_Y                180
#define LDONE_FILE_Y                200
#define LDONE_PATH_Y                220

#define LDONE_BTN_Y                 (GUI_SCREEN_HEIGHT - 36)

#define LDONE_COLOR_LABEL           make_color(0x99, 0x99, 0x99)
...
static u8 ldone_bat_level_from_percent(u8 percent)
{
    if (percent <= 20) {
        return 1;
    }
...
static void ldone_update_battery(void)
{
    f_ldone_t *f = (f_ldone_t *)func_cb.f_cb;
    u8 level;
    ...
    level = ldone_bat_level_from_percent(sys_cb.vbat_percent);
...
static void ldone_update_display(void)
{
    f_ldone_t *f = (f_ldone_t *)func_cb.f_cb;
    char buf[48];
...
compo_form_t *func_latest_n_day_done_page_form_create(void)
{
    ...
    compo_picturebox_set_pos(f->pic_divider, GUI_SCREEN_CENTER_X, LDONE_DIVIDER_Y);

    f->txt_title = compo_textbox_create(frm, 24);
    compo_textbox_set_location(f->txt_title, GUI_SCREEN_CENTER_X, LDONE_TITLE_Y, 0, 0);
    ...
    f->bat_level = ldone_bat_level_from_percent(sys_cb.vbat_percent);
    f->pic_bat = compo_picturebox_create(frm, s_bat_level_res[f->bat_level - 1]);
    compo_picturebox_set_pos(f->pic_bat, LDONE_BAT_X, LDONE_TITLE_Y);

    /* 中间：完成勾选图标 */
    f->pic_ok = compo_picturebox_create(frm, UI_BUF_IMAGE_BIN_CHECK_OUT_CORRECT_BIN);
    compo_picturebox_set_pos(f->pic_ok, GUI_SCREEN_CENTER_X, LDONE_ICON_Y);

    /* 备份完成 */
    txt = compo_textbox_create(frm, 16);
    compo_textbox_set_location(txt, GUI_SCREEN_CENTER_X, LDONE_STA_Y, 0, 0);
    ...
    compo_textbox_set_location(f->txt_size, GUI_SCREEN_CENTER_X, LDONE_SIZE_Y, 0, 0);
    ...
    compo_textbox_set_location(f->txt_file, GUI_SCREEN_CENTER_X, LDONE_FILE_Y, 0, 0);
    ...
    compo_textbox_set_location(f->txt_path, GUI_SCREEN_CENTER_X, LDONE_PATH_Y, 0, 0);
    ...
    compo_textbox_set_forecolor(f->txt_path, LDONE_COLOR_LABEL);

    /* 底部完成按钮 */
    f->pic_btn = compo_picturebox_create(frm, UI_BUF_IMAGE_BIN_LONG_BOTTON_BLUE_UNCLICK_BIN);
    compo_picturebox_set_pos(f->pic_btn, GUI_SCREEN_CENTER_X, LDONE_BTN_Y);

    txt = compo_textbox_create(frm, 8);
    compo_textbox_set_location(txt, GUI_SCREEN_CENTER_X, LDONE_BTN_Y, 0, 0);
    compo_textbox_set_autosize(txt, true);
    compo_textbox_set_align_center(txt, true);
    ...
    f->btn_press = 0;
    ldone_update_display();

    return frm;
}

static void func_latest_n_day_done_page_process(void)
{
    f_ldone_t *f = (f_ldone_t *)func_cb.f_cb;
    ...
    ldone_update_battery();
    func_process();
}

static void func_latest_n_day_done_page_message(size_msg_t msg)
{
    f_ldone_t *f = (f_ldone_t *)func_cb.f_cb;
    point_t pt;
...
void func_latest_n_day_done_page_enter(void)
{
    ...
    func_cb.f_cb = func_zalloc(sizeof(f_ldone_t));
    f = (f_ldone_t *)func_cb.f_cb;

    f->days = backup_param.latest_days ? backup_param.latest_days : 1;
    f->card_total = 2;
    f->file_cnt = 412;
    strcpy(f->size_str, "8.6 GB");
    ...
    func_cb.frm_main = func_latest_n_day_done_page_form_create();
}
...
void func_latest_n_day_done_page(void)
{
    printf("%s\n", __func__);
    func_latest_n_day_done_page_enter();
    while (func_cb.sta == FUNC_LATEST_N_DAY_DONE_PAGE)
    {
        func_latest_n_day_done_page_process();
        func_latest_n_day_done_page_message(msg_dequeue());
    }
    func_latest_n_day_done_page_exit();
}
